...
public class TaiKhoanDAO extends InterfaceDAO<TaiKhoan, String> {
	...
	private final String SELECT_ALL_SQL = "SELECT tk.*, nv.hoTen, nv.sdt, nv.gioiTinh, nv.namSinh, nv.ngayVaoLam, vt.ten as tenVT "
			+ "FROM TaiKhoan tk " + "INNER JOIN NhanVien nv ON tk.idNV = nv.idNV "
	...
	private final String SELECT_BY_ID = "SELECT tk.*, nv.hoTen, nv.sdt, nv.gioiTinh, nv.namSinh, nv.ngayVaoLam, vt.ten as tenVT "
			+ "FROM TaiKhoan tk " + "INNER JOIN NhanVien nv ON tk.idNV = nv.idNV "
			+ "INNER JOIN VaiTro vt ON tk.idVT = vt.idVT " + "WHERE tk.idTK = ?";

	private final String SELECT_BY_USERNAME = "SELECT tk.*, nv.hoTen, nv.sdt, nv.gioiTinh, nv.namSinh, nv.ngayVaoLam, vt.ten as tenVT "
			+ "FROM TaiKhoan tk " + "INNER JOIN NhanVien nv ON tk.idNV = nv.idNV "
			+ "INNER JOIN VaiTro vt ON tk.idVT = vt.idVT " + "WHERE tk.username = ?";

	@Override
	public void create(TaiKhoan e) {
		JDBCConnection.update(INSERT_SQL, e.getId(), e.getUsername(), e.getPassword(), e.getNhanVien().getId(),
				e.getVaiTro().getId());
	}

	@Override
	public void update(TaiKhoan e) {
		JDBCConnection.update(UPDATE_SQL, e.getUsername(), e.getPassword(), e.getNhanVien().getId(),
				e.getVaiTro().getId(), e.getId());
	}
