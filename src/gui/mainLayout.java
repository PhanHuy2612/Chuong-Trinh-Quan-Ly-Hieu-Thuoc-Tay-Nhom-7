...
public class mainLayout extends Application {
    ...
    private void initMenuActions() {
        // Dashboard
        menuActions.put("Dashboard", () -> {
            try {
                // Nếu đã có instance Dashboard, hủy nó
                if (currentDashboard != null) {
                    currentDashboard.stop();
                }

                // Tạo Dashboard mới
                currentDashboard = new Dashboard();
                loadScreen(currentDashboard.getRoot());
            } catch (Exception e) {
                e.printStackTrace();
                showPlaceholder("Lỗi tải Dashboard: " + e.getMessage());
            }
        });
        ...
        // Thống kê - hỗ trợ cả Nhân viên và Quản lý
        menuActions.put("Báo cáo doanh thu", () -> {
            try {
                gui_dialog.frmThongKe thongKe = new gui_dialog.frmThongKe(taiKhoan.getPhanQuyen());
                loadScreen(thongKe.getRoot());
            } catch (Exception e) {
                e.printStackTrace();
                showPlaceholder("Lỗi tải thống kê: " + e.getMessage());
            }
        });

        menuActions.put("Thống kê", () -> {
            try {
                gui_dialog.frmThongKe thongKe = new gui_dialog.frmThongKe(taiKhoan.getPhanQuyen());
                loadScreen(thongKe.getRoot());
            } catch (Exception e) {
    ...
    private void createSidebarAndHeader() {
        ...
        if (taiKhoan.getPhanQuyen() == PhanQuyen.QUAN_LY) {
            ...
            sidebarQL.setMenuAction("Dashboard", menuActions.get("Dashboard"));
            ...
            if (taiKhoan.getPhanQuyen() == PhanQuyen.DUOC_SI) {
                sidebarMenu.setMenuAction("Dashboard", menuActions.get("Dashboard"));
                ...
                sidebarMenu.setMenuAction("Thống kê", menuActions.get("Thống kê"));
            } else if (taiKhoan.getPhanQuyen() == PhanQuyen.DUOC_SI) {
                sidebarMenu.setMenuAction("Dashboard", menuActions.get("Dashboard"));
                sidebarMenu.setMenuAction("Quản lý bán thuốc", menuActions.get("Quản lý bán thuốc"));
    ...
    private void openDefaultScreen() {
        ...
        if (taiKhoan.getPhanQuyen() == PhanQuyen.QUAN_LY ||
            taiKhoan.getPhanQuyen() == PhanQuyen.DUOC_SI) {
            defaultAction = menuActions.get("Dashboard");
        } else {
